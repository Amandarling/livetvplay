...
export default function App() {
  ...
  return (
    <div className="font-sans select-none h-[100dvh] w-full bg-bg-deep text-white flex flex-col overflow-hidden relative">
      ...
      {selectedStream && auth && (
        <VideoPlayer
          url={selectedStream.stream_type === 'live'
            ? (selectedStream.direct_source ? xtreamService.getProxiedUrl(selectedStream.direct_source) : xtreamService.getStreamUrl(auth.user_info.username, auth.user_info.password, selectedStream.stream_id, 'm3u8'))
            : (selectedStream.stream_type === 'movie'
          ...
          type={selectedStream.stream_type as any}
          isFavorite={favorites.some(f => f.stream_id === selectedStream.stream_id)}
          onClose={() => setSelectedStream(null)}
          onToggleFavorite={() => toggleFavorite(selectedStream)}
        />
      )}
